...
if($step == '1') {
    ...
    $accessevns[] = $checkarr;
    //Operimizer
    if(phpversion() >= '5.2.0' && phpversion() < '5.3.0') {
        $checkarr = array('name'=>'Zend Operimizer','access'=>'3.3.0');
        //OPTIMIZER_VERSION
        if(defined('OPTIMIZER_VERSION') && OPTIMIZER_VERSION >= '3.3.0') {
            $checkarr['currently'] = '<span class="font_3">√ '.OPTIMIZER_VERSION.'</span>';
        } else {
            $next_enable = ' disabled';
            $checkarr['currently'] = '<span class="font_1">× 版本过低或未安装</span>';
        }
        $accessevns[] = $checkarr;
    }
    //文件目录属性监测
    ...
    if($error) {
        ...
        $_G['db']->query("REPLACE INTO {$_G['dns']['dbpre']}config (variable,value,module) VALUES('version','$version','modoer')");
        $_G['db']->query("REPLACE INTO {$_G['dns']['dbpre']}config (variable,value,module) VALUES('build','$build','modoer')");
        ...
        $_G['db']->query("INSERT INTO {$_G['dns']['dbpre']}admin (adminname,email,password,admintype,is_founder,logintime,loginip,logincount,mymodules) 
                VALUES ('$username','$email','".md5($password)."','1','Y','0','','0','')");

        //更新模块版本
        $modules = array('item', 'review', 'article', 'coupon', 'exchange', 'comment', 'card', 'group', 'member', 'space', 
            'link', 'adv', 'sms', 'pay', 'mobile');
        $_G['db']->from('dbpre_modules');
        $_G['db']->where('flag', $modules);
        $r = $_G['db']->get();
        while ($v = $r->fetch_array()) {
            $newmodule = array();
            @include MUDDER_CORE . 'modules' . DS . $v['flag'] . DS . 'install' . DS . 'info.php';
            if(!$newmodule) continue;
            foreach(array('reliant','introduce','version','releasetime','checkurl') as $key) {
                if(isset($newmodule[$key])) {
                    $_G['db']->set($key, $newmodule[$key]);
                }
            }
            $_G['db']->from('dbpre_modules');
            $_G['db']->where('flag', $v['flag']);
            $_G['db']->update();
        }
...
<h1 id="subject">
    <span id="copyright">Copyright &copy; 2007-2012 <a href="http://www.modoer.com/" target="_blank">Moufer Studio</a></span>
...
<table>
	...
	<tr><td>Cookie作用域:</td><td><input type="text" name="cookiedomain" class="t_input" value="<?php echo $cookiedomain?>" /></td><td>如需要使用二级域名城市分站，请填写主域名，反之留空</td></tr>
</table>
<div id="comment">如果您无法确认以上的配置信息，请与您的服务商联系。</div>
