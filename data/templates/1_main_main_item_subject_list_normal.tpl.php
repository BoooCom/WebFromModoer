...
<script type="text/javascript" src="<?=URLROOT?>/static/javascript/swfobject.js"></script><?php $index=0; if($list) { while($val = $list->fetch_array()) { ?>
...
<? if(is_array($reviewpot)) { foreach($reviewpot as $_val) { ?>
                            <?=$_val['name']?>:<span 
                             class="font_2" style="font-size:16px;"><?php echo cfloat($val[$_val['flag']]); ?></span>
