...
<?php $_QUERY['get_val3']=$_G['datacall']->datacall_get('subject',array('select'=>"sid,name,subname,reviews,thumb,best,description",'city_id'=>"_NULL_CITYID_",'pid'=>$val['catid'],'rand'=>1,'rows'=>2,'cachetime'=>500,),'item');
...
" /></a>
                            ...
                            <div class="class-finer-subject-info">
                                <? if($val3['reviews']) { ?>
<em>好评度<span class="font_1"><?php echo round($val3['best']/($val3['reviews']*2)*100); ?>%</span></em><? } ?>
    ...
    <div id="class_right">
        <div class="mainrail rail-border-3">
            ...
            <ul class="rail-list">
<?php $_QUERY['get_val']=$_G['datacall']->datacall_get('review',array('city_id'=>"_NULL_CITYID_",'orderby'=>"posttime DESC",'rows'=>8,'cachetime'=>500,),'review');
if(is_array($_QUERY['get_val']))foreach($_QUERY['get_val'] as $val_k => $val) { ?>
