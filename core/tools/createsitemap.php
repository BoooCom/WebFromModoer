...
class msm_tool_createsitemap extends msm_tool {
    ...
    protected $descrption = '生成Sitemap文件(XML格式)，让搜索引擎更易于抓取网站URL内容。xml文件存在在 ./data 目录内，
    ...
    public function run() {
        ...
        $this->_sitemap->add(url("index",'',true));
    ...
    private function _url_1() {
        ...
        while ($v = $r->fetch_array()) {
            $url = url("item/detail/id/$v[sid]",'',true);
            $this->_sitemap->add($url, $v['addtime']);
        }
        $r->free_result();
    }
    ...
    private function _url_2() {
        ...
        while ($v = $r->fetch_array()) {
            ...
            $this->_sitemap->add($url, $v['dateline']);
        }
        $r->free_result();
    }
    ...
    private function _url_4() {
        if(!check_module('discussion')) return;
        $r = $this->_db->from('dbpre_discussion_topic')
        ...
        if(!$r) return;
        while ($v = $r->fetch_array()) {
            $url = url("discussion/topic/id/$v[tpid]",'',true);
            $this->_sitemap->add($url, $v['replytime']?$v['replytime']:$v['dateline']);
        }
        $r->free_result();
    }
