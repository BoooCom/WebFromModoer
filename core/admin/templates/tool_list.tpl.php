...
<div id="body">
    <div class="space">
        ...
        <ul class="cptab">
            <li<?if(!$_GET['type']):?> class="selected"<?endif;?>>
                <a href="<?=cpurl($module,$act)?>" onfocus="this.blur()">全部脚本</a>
            </li>
            <li<?if($_GET['type']=='delete'):?> class="selected"<?endif;?>>
                <a href="<?=cpurl($module,$act,$op,array('type'=>'delete'))?>" onfocus="this.blur()">清理类</a>
            </li>
            <li<?if($_GET['type']=='repair'):?> class="selected"<?endif;?>>
                <a href="<?=cpurl($module,$act,$op,array('type'=>'repair'))?>" onfocus="this.blur()">修复类</a>
            </li>
            <li<?if($_GET['type']=='rebuild'):?> class="selected"<?endif;?>>
                <a href="<?=cpurl($module,$act,$op,array('type'=>'rebuild'))?>" onfocus="this.blur()">重建类</a>
            </li>
            <li<?if($_GET['type']=='other'):?> class="selected"<?endif;?>>
                <a href="<?=cpurl($module,$act,$op,array('type'=>'other'))?>" onfocus="this.blur()">其他类</a>
            </li>
        </ul>
        <table class="maintable" border="0" cellspacing="0" cellpadding="0" trmouse="Y">
            <?foreach($tools as $key => $tool):?>
            <?
                if($type && $tool->get_type() != $type) continue;
            ?>
            <tr>
                <td width="*">
