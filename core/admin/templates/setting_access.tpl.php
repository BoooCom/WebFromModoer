...
<?=form_begin(cpurl($module,$act,$op))?>
    <div class="space">
        ...
        <table class="maintable" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr>
                <td class="altbg1"><strong>可引用Flash域名:</strong>仅允许设定的域名的flash显示在网站内，可以防止未知网站的恶意Flash进行跨站攻击。
                    一行一个域名（只需要填写顶级域名即可，不需要www.，例如modoer.com）。</td>
                <td width="*"><textarea name="setting[swf_domain]" rows="6" cols="40" class="txtarea"><?=$config['swf_domain']?></textarea></td>
            </tr>
            <tr>
                <td class="altbg1"><strong>IP 访问列表:</strong>只有当用户处于本列表中的 IP 地址时才可以访问本网站，列表以外的地址访问将视为 IP 被禁止，仅适用于诸如企业、学校内部网站等极个别场合。本功能对管理员没有特例，如果管理员不在此列表范围内将同样不能登录，请务必慎重使用本功能。每个 IP 一行，请输入完整地址，例如 "192.168.1.241"，留空为所有 IP 除明确禁止的以外均可访问；支持通配符*，来指定IP段，下面几个都支持</td>
