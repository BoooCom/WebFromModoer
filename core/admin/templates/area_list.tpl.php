...
<div id="body">
    ...
    <center>
        ...
        <? if($level>1) :?>
        <input type="button" value=" 新建本级地区 " class="btn" onclick="jslocation('<?=cpurl($module,$act,'add',array('pid'=>$pid))?>')" />
        <input type="button" value=" 返回上一级 " class="btn" onclick="jslocation('<?=cpurl($module,$act,'',array('pid'=>$detail['pid']))?>')" />
        <?else:?>
        <input type="button" value=" 新建城市 " class="btn" onclick="jslocation('<?=cpurl($module,$act,'add',array('level'=>'1'))?>')" />&nbsp;
        <input type="button" value=" 导入数据 " class="btn" onclick="jslocation('<?=cpurl($module,$act,'import')?>')" />&nbsp;
