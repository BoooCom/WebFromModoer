...
case 'edit':
    ...
    if(!$admin->is_founder && $admin->adminid != $_GET['adminid']) redirect('global_op_access');
...
case 'post':
    if($_POST['do'] == 'edit') {
        ...
        if(!$admin->is_founder && $admin->adminid != $adminid) redirect('global_op_access');
...
default:
    if(!$admin->is_founder) {
        header("Location:"  . cpurl($module,$act,'edit',array('adminid'=>$admin->adminid)));
        exit;
    }
    $op = 'list';
    $offset = 20;
...
function check_sub_module($module, &$mymodules) {
    foreach ($mymodules as $value) {
