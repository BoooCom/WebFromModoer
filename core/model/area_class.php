...
class msm_area extends ms_model {
        ...
        if($edit) {
            ...
            if(isset($post['pid'])) {
                if(!$post['pid']) {
                    ...
                    if(strposex($post['name'],'|')) {
                        $aid = $this->add_batch($post['name'],$post['pid'],$post['mappoint']);
                        return $aid;
                    }
    ...
    function add_batch($name,$pid,$mappoint='') {
        if(!$parent = $this->read($pid)) {
            redirect('admincp_area_empty_pid');
        } elseif($parent['level'] == '3') {
            redirect('admincp_area_level_max');
        }
        $leve = (int) $parent['level'] + 1;
        $ns = explode('|', $name);
        foreach ($ns as $value) {
            if(!$value) continue;
            $array = array(
            ...
            $aid = parent::save($array, null ,false);
            $attid = $this->loader->model('item:att_list')->save($aid, $value, 'area');
            if($attid) {
                $this->db->from($this->table)->set('attid',$attid)->where('aid',$aid)->update();
            }
        }
    ...
    function write_cache($update_jscache_flag = FALSE) {
        $this->_write_cache();
        //js地区文件更新标识
    ...
    function _get_cache($key) {
        ...
        $this->db->order_by(array('level'=>'ASC','listorder'=>'ASC'));
        if($query = $this->db->get()) {
            $area = $file = $level2 = $level3 = $rel = false;
            while($val = $query->fetch_array()) {
                $rel[$val['aid']] = $val['pid'] . ':' . $val['level'];
                if($val['level']=='1') { //城市
                    if($val['config']) $val['config'] = unserialize($val['config']);
        ...
        $cache = ms_cache::factory();
        $cache->write('modoer_area', $area);
        if(!$key) $result = $area;
        $cache->write('modoer_area_rel', $rel);
        if($key == 'rel') $result = $rel;
        foreach($file as $k => $v) {
            $cache->write('modoer_area_' . $k, $v);
            if($key == $k) $result = $v;
        }
        return $result;
    }
