...
*/
class msm_tuan_category extends ms_model {

    var $table = 'dbpre_tuan_category';
	var $key = 'catid';
    var $model_flag = 'tuan';
    ...
    function check_post(& $post, $edit = false) {
        if(!$post['name']) redirect('tuan_category_name_empty');
    }
    ...
    function rebuild($catids = null) {
        ...
        while($v=$r->fetch_array()) {
            ...
            $this->db->where_in('status', array('new','succeeded','lose'));
            $count = $this->db->count();
            $this->db->from($this->table);
    ...
    function write_cache($return = false) {
        ...
        ms_cache::factory()->write('tuan_category', $result);
