...
class msm_ucenter_user extends msm_ucenter_userbase {
    ...
    function login($username, $password, $life=2592000) {
        ...
        //本地手机登陆判断
        if(!$result && $this->modcfg['mobile_login']&&preg_match("/^[0-9]{9,15}$/", $username)) {
            if($member = $this->read($username, 2)) {
                $result = uc_user_login($member['username'], $password);
                if($result) $username = $member['username'];
            }
        }
        if(!$result) redirect('member_login_lost');
    ...
    function auto_login() {
        ...
        if(_input('in_ajax') && $_POST['auto_login_hash'] != '') {
            list($c_uid, $c_hash) = explode('|',base64_decode($_POST['auto_login_hash']));
        }
    ...
    function register(& $post, $activation=FALSE) {
        if($this->modcfg['closereg']) redirect('member_reg_closed');
        if($this->modcfg['mobile_verify'] && !$post['mobile']) redirect('member_reg_mobile_verify_invalid');
        if($this->registered_again()) redirect('member_post_registered_again');
        $passport = array();
        $post['logintime'] = 0;
        $post['regdate'] = $this->timestamp;
    ...
    //第三方账号自动注册
    function passport_auto_register($username, $passport_type, $passport_id) {
        //用户名检测
        if($this->check_username_exists($username)) {
            $username = substr($this->global['timestamp'], -2) . '_' . $username;
        }
        $post = array();
        $post['username'] = $username;
        $post['email'] = $passport_type.$passport_id.'@'.get_fl_domain(); //因为系统必须要email，这里随机分配email
        $post['password'] = $post['password2'] = random();
        $post['logintime'] = $post['regdate'] = $this->timestamp;
        $post['regip'] = $post['loginip'] = $this->global['ip'];
        $post['logincount'] = 1;
        $post['groupid'] = 10;
         $this->global['passport_auto_reg'] = true;
        $this->uid = (int) $this->save($post);
        $this->hash = create_formhash($this->uid, $username, '');
        $this->isLogin = TRUE;
        //第三方账号记录
        $passport = array();
        $passport['psname'] = $passport_type;
        $passport['psuid'] = $passport_id;
        $passport['uid'] = $this->uid;
        $PT =& $this->loader->model('member:passport');
        $PT->save($passport);
    ...
    //判断密码
    function check_passpord($password) {
        list($tmp_uid) = uc_user_login($this->uid, $password, true);
        return $tmp_uid > 0;
    }
