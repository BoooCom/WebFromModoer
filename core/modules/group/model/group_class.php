...
class msm_group extends ms_model {
    ...
    function init_field() {
        $this->add_field('catid,groupname,des,tags,sid');
        $this->add_field_fun('catid,uid,sid', 'intval');
        $this->add_field_fun('groupname,des,icon', '_T');
    }

    //通过标签查询
    function find_tag($select,$tagname,$orderby=array('members'=>'DESC'),$start=0,$offset=20) {
        $this->db->where('gt.tagname',$tagname);
