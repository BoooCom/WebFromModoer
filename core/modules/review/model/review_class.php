...
class msm_review extends ms_model {
    ...
    function save($post, $rid = null) {
        ...
        if(!$is_edit && $post['status']) {
            ...
            $this->_feed($rid);
            //葫芦
            $this->loader->model('item:gourd')->update_progress($detail['id']);
            //同步到第三方平台
            if($_POST['share']) {
    ...
    function checkup($rids) {
        ...
        if($subjects) foreach($subjects as $idtype => $subject) {
            ...
            foreach($subject as $val) {
                ...
                //葫芦
                $this->loader->model('item:gourd')->update_progress($val['id']);
            }
        }
    }
    ...
    function add_user_point($uid, $num = 1) {
        ...
        $this->loader->model('member:point')
            ->update_point($uid, 'add_review', 0, $num, FALSE, array('reviews'=>array('set_add',array($num))));
    }

    function dec_user_point($uid, $num = 1) {
        if(!$uid) return;
        $this->loader->model('member:point')
            ->update_point($uid, 'add_review', TRUE, $num, FALSE, array('reviews'=>array('set_add',array($num))));
    }
    ...
    //提示权限信息
    function redirect_access($result, $not_login_url = 'member/login', $ret_rul = 'javascript:history.go(-1);') {
        $lang = lang('review_access_'.$result['code']);
        $lang = str_replace('{S}', $result['extra'], $lang);
        if($result['code']=='-2') {
            location(url($not_login_url));
            //redirect($lang, url($not_login_url));
        } else {
            redirect($lang, $ret_rul);
        }
    }
    ...
    function _delete($where, $update_total = TRUE, $delete_point = FALSE) {
        ...
        while($value=$q->fetch_array()) {
            ...
            if($value['status']) {
                //点评对象统计
                if(!is_array($subjects[$value['idtype']])) $subjects[$value['idtype']] = array();
                if($update_total && !in_array($value['id'], $subjects[$value['idtype']])) {
                    $subjects[$value['idtype']][$value['id']] = array(
                        'pid' => $value['pcatid'],
                        'idtype' => $value['idtype'],
                        'id' => $value['id'],
                    );
    ...
    function _feed_subject(&$FEED, &$detail) {
        ...
        $param['username'] = '';
        $param['icon'] = lang('review_feed_add_icon');
        $param['sid'] = $detail['id'];
        $param['city_id'] = (int)$subject['city_id'];
