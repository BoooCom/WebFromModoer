...
class msm_review_respond extends msm_item_itembase {
	...
	function add_user_point($uid, $num = 1) {
		...
		$this->loader->model('member:point')
            ->update_point($uid, 'add_respond', FALSE, $num, FALSE, array('responds'=>array('set_add',array($num))));
	}

	function dec_user_point($uid, $num = 1) {
        if(!$uid) return;
		$this->loader->model('member:point')
            ->update_point($uid, 'add_respond', TRUE, $num, FALSE, array('responds'=>array('set_add',array($num))));
	}
