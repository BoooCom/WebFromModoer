...
class msm_member_user extends msm_member_userbase {
    ...
    function login_passport($uid, $passport_type='', $passport_id='', $life=2592000) {
    ...
    function login($username, $password, $life=2592000) {
        ...
        if(!$member = $this->read($username, 1)) {
            if($this->modcfg['mobile_login']&&preg_match("/^[0-9]{9,15}$/", $username)) {
                $member = $this->read($username, 2);
            }
            if(!$member) {
                $this->_init_variable();
                return FALSE;
    ...
    function passport_auto_register($username, $passport_type, $passport_id) {
        ...
        $post['email'] = $passport_type.$passport_id.'@'.get_fl_domain();
        $post['password'] = $post['password2'] = random();
        $post['logintime'] = $post['regdate'] = $this->timestamp;
        $post['regip'] = $post['loginip'] = $this->global['ip'];
        $post['logincount'] = 1;
        ...
        $password = md5($post['password']);
        $this->uid = (int) $this->save($post);
        $this->hash = create_formhash($this->uid, $post['username'], $password);
        $this->isLogin = TRUE;
        $this->_save_login(3600 * 24 * 30);
