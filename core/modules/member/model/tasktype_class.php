...
class msm_member_tasktype extends ms_model {
    ...
    function instantiate($flag) {
        ...
        if(!is_file($file)) {
            if($this->in_admin) {
                redirect(lang('global_file_not_exist', str_replace(MUDDER_CORE,'./',$file)));
            } else {
                return false;
            }
        }
    ...
    function write_cache($return = FALSE) {
        $result = $this->_load_list();
        ms_cache::factory()->write('member_tasktype', $result);
