...
class msm_member_usergroup extends ms_model {
    ...
    function write_cache($return = FALSE) {
        ...
        while($value = $r->fetch_array()) {
            ...
            $cache->write('member_' . $this->cache_name . '_' . $value['groupid'], $value);
        ...
        $cache->write('member_' . $this->cache_name, $result);
        //write_cache($this->cache_name, arrayeval($result), $this->model_flag);
        if($return) return $result;
    }

    function write_cache_access($groupid, $return = FALSE) {
        if(!$result = $this->read($groupid)) return;
        $cache = ms_cache::factory()->write('member_' . $this->cache_name . '_' . $value['groupid'], $result);
