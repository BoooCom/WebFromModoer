...
if($_POST['dosubmit']) {
    ...
    if($inviter_uid = _cookie('inviter_uid', 0, MF_INT_KEY)) {
        ...
        if($IV->chack_enable($inviter_uid)) {
            $IV->add($inviter_uid);
        }
    ...
    switch ($passport_type) {
        ...
        case 'taobao':
            ...
            $passport_id = $_G['cookie']['passport_taobao_user_id'];
            if($passport_id) {
                $username = $_G['cookie']['passport_taobao_nick'];
                $passport = true;
                $access_token = '';
                $expires_in = '0';
            } else {
    ...
    if($passport) {
        ...
        if($uid = $PT->get_uid($passport_type, $passport_id)) {
            $sync = $user->login_passport($uid, $passport_type, $passport_id, 3600*24);
            //update
            $PT->update_access_token($uid, $psname, $access_token, $expires_in);
            redirect(lang('member_login_succeed') . $sync, url("member/index"));
        }
        if($MOD['passport_reg_skip'] == 'force' || ($MOD['passport_reg_skip'] && $_GET['skip']=='Y')) {
            //强制跳过（自动）本地注册
            $sync = $user->passport_auto_register($username, $passport_type, $passport_id);
            //task apply
            $_G['loader']->model('member:task')->automatic_apply();
            redirect(lang('member_reg_succeed') . $sync, url('member/index'));
        }
    ...
    //invite
    $inviter_uid = _get('intive', 0, MF_INT_KEY);
    $IV = $_G['loader']->model('member:invite');
    if(!$invite = $IV->chack_enable($inviter_uid)) {
        $invite_message = $IV->get_message();
    } else {
        set_cookie('inviter_uid', $inviter_uid);
    }
