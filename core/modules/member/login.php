...
case 'login':
    ...
    redirect(lang('member_login_succeed') . $sync, $url?$url:$forward);
    break;
case 'passport_login':
...
case 'passport_callback':
    ...
    switch($psname) {
        ...
        case 'taobao':
            ...
            $o = new TaobaoOAuth($MOD['passport_taobao_appkey'], $MOD['passport_taobao_appsecret']);
            $callbackurl = str_replace('&amp;','&',url("member/login/op/passport_callback/type/taobao",'',1,1));
            $token = $o->getAccessToken( $_REQUEST['top_parameters'] , $callbackurl);
            $access_token = $token->access_token;
            $expires_in = $_G['timestamp'] + $token->expires_in;
    ...
    if(!$passport_id) {
        ...
        $sync = $user->login_passport($uid, $psname, $passport_id, 3600*24);
        //update
