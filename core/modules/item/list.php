...
//获取SID列表
if($list && $type == 'pic') {
    $sids = $arrlist = $revews = array();
    while($val = $list->fetch_array()) {
        $sids[] = $val['sid'];
        $arrlist[] = $val;
    }
    $list->free_result();
    $sql = "SELECT a.rid,a.id,a.content,a.uid,a.username FROM ".$_G['dns']['dbpre']."review a, ".$_G['dns']['dbpre']."review b WHERE a.id = b.id 
        AND a.id IN ( '".implode("','", $sids)."' ) 
        AND a.status = 1 
        AND a.`posttime` <= b.`posttime` 
        GROUP BY a.rid, a.id, a.`posttime` 
        HAVING COUNT( b.rid ) <=4 
        ORDER BY a.id, a.`posttime` DESC";
    $reviewlist = $_G['db']->query($sql);
    if($reviewlist) {
        while($val = $reviewlist->fetch_array()) {
            $revews[$val['id']][] = $val;
        }
        $reviewlist->free_result();
    }
...
$seo_tags['root_area_name'] = $paid>0?display('modoer:area',"aid/$paid"):($aid>0?display('modoer:area',"aid/$aid"):'_');
...
$seo_tags['page'] = $_GET['wfpage'] ? $_GET['wfpage'] : $_GET['page'];
...
$seo_list_title = $MOD["seo_list_{$pid}_title"] ? $MOD["seo_list_{$pid}_title"] : $MOD['seo_list_title'];
$seo_list_keywords = $MOD["seo_list_{$pid}_keywords"] ? $MOD["seo_list_{$pid}_keywords"] : $MOD['seo_list_keywords'];
$seo_list_description = $MOD["seo_list_{$pid}_description"] ? $MOD["seo_list_{$pid}_description"] : $MOD['seo_list_description'];

!$seo_list_title && $seo_list_title = '{root_category_name},{area_name},{city_name}';
!$seo_list_keywords && $seo_list_keywords = '{root_category_keywords}';
!$seo_list_description && $seo_list_description = '{root_category_description}';

$_HEAD['title'] = parse_seo_tags($seo_list_title, $seo_tags);
$_HEAD['keywords'] = parse_seo_tags($seo_list_keywords, $seo_tags);
$_HEAD['description'] = parse_seo_tags($seo_list_description, $seo_tags);
