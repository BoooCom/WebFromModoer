...
class display_item {
    ...
    function category_nav($params) {
        $subject = $params['subject'];
        if(!$subject||!is_array($subject))return '';
        $split = $params['split'] ? $params['split'] : '&nbsp;&raquo;&nbsp;';
        $subsplit = $params['subsplit'] ? $params['subsplit'] : '&nbsp;,&nbsp;';
        $urlpath = array();
        $urlpath[] = url_path(display('item:category',"catid/$subject[pid]"), url("item/list/catid/$subject[pid]"));
        if($subject['catid']!=$subject['pid']) {
            $urlpath[] = url_path(display('item:category',"catid/$subject[catid]"), url("item/list/catid/$subject[catid]"));
        }
        if($subject['sub_catids']) {
            $sub_catids = explode('|',substr($subject['sub_catids'],1,-1));
            if($sub_catids) {
                $subpath = array();
                foreach ($sub_catids as $sub_catid) {
                    $subpath[] = url_path(display('item:category',"catid/$sub_catid"), url("item/list/catid/$sub_catid"));
                }
                $urlpath[] = implode($subsplit, $subpath);
            }
        }
        return implode($split, $urlpath);
    }
