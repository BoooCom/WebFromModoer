...
class query_item {
    ...
    function top($params) {
        ...
        $cachename = create_identifier($params);
        $dirname = substr($cachename, -2);

        $cachedir = $cachedir . $dirname;
        if(!is_dir(MUDDER_ROOT . $cachedir)) {
            if(!mkdir(MUDDER_ROOT . $cachedir, 0777)) {
                exit("Current picture path '".str_replace(MUDDER_ROOT, './', $cachedir)."' have no access!");
            }
        }

        $lastcache =  MUDDER_ROOT . $cachedir . DS . $cachename . '.php';
        $comparecache = MUDDER_ROOT . $cachedir . DS . $cachename . '_old.php';
        ...
        if(!$file_exists || $compare) {
            ...
            $index = 1;
            $db->from($I->table);
            $db->select('sid,pid,catid,name,subname,thumb,reviews,avgprice,favorites,avgsort');
