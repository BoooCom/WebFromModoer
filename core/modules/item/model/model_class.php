...
class msm_item_model extends ms_model {
    ...
    function add($post, $export=false) {
        ...
        if($import_array['att_cat']) {
            ...
            $attcats = $A->import($import_array['att_cat'],_post('model_import_jump_sname'));
        }
        // 在村需要导入的标签组
        if($import_array['taggroup']) {
            $TG =& $this->loader->model('item:taggroup');
            $taggroups = $TG->import($import_array['taggroup'],_post('model_import_jump_sname'));
        }
    ...
    function write_cache($modelid = null, $return = FALSE) {
        ...
        while($value = $row->fetch_array()) {
            $models[$value['modelid']] = $value['name'];
            if(!$modelid || $modelid == $value['modelid']) {
                ms_cache::factory()->write('item_model_' . $value['modelid'], $value);
        ...
        $row->free_result();
        ms_cache::factory()->write('item_model', $models);
        //write_cache('model', arrayeval($result), $this->model_flag);
        if($return && $modelid) return $result;
        if($return && !$modelid) return $models;
    }
