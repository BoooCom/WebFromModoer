...
class msm_item_fieldform extends msm_fieldform {
	...
	function _text($val) {
		...
		$check_exists_btn = " <button type=\"button\" onclick=\"item_subject_check_exists('{$this->ctrid}',"._G('city','aid').");\">".lang('item_check_subject_title')."</button>";
        $content = "<tr>\r\n";
    ...
    function _category($val) {
        ...
        $option = $val!=ITEM_PID ? form_item_category_sub($val, $sub_catids) : '';
        $content .= "\t<select id=\"category_3\" name=\"t_item[sub_catids][]\" multiple=\"true\">$option</select>\r\n";
...
/*        $tpllist = $this->loader->variable('templates');
    ...
    function _subject($val) {
        ...
        $content .= "\tcatid:'{$this->field['config']['categorys']}',\n";
