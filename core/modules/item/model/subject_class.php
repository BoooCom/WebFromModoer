...
class msm_item_subject extends msm_item_itembase {
    ...
    function read_random($where=null,$num=1,$return_rs=false) {
        $this->db->from($this->table);
    ...
    function getcount($where) {
        ...
        */
        ...
        if($atts) {
            ...
            $num = count($attlist);
            if($num>1) {
                $this->db->where_exist("SELECT 1 FROM dbpre_subjectatt st FORCE INDEX (sid) WHERE s.sid=st.sid AND attid IN (".implode(',',$attlist).") 
                    GROUP BY st.sid HAVING COUNT(st.sid)=$num");
            } else {
                $this->db->join($this->table, 's.sid','dbpre_subjectatt', 'st.sid', 'LEFT JOIN');
                $this->db->where('attid', array_pop($attlist));
            }
    ...
    function save($post, $sid = null) {
        ...
        */
        ...
        //附表
		$i_f = array('forumid','groupid');
        foreach($i_f as $v) {
            if(isset($post[$v])) $field_post[$v] = (int)$post[$v];
        }
        ...
        if($edit) {
            ...
            if($field_post) {
                if($this->db->from($table)->where('sid',$sid)->count()>0) {
                    $this->db->from($table);
    ...
    function set_owner($sid, $username, $expirydate = '', $update_subject = true, $show_error = true, $is_username = true) {
        ...
        if($expirydate) {
            ...
            if($expirydate < strtotime(date('Y-m-d',$this->global['timestamp']))) {
                if(!$show_error) return;
    ...
    //检查一个主题的管理员管理权限是否到期
    function check_owner_expirydate($sid) {
        $this->db->from('dbpre_mysubject');
        $this->db->where('sid', $sid);
        $owner = $this->db->get_one();
        if(!$owner || !$owner['expirydate']) return;
        $today = strtotime(date('Y-m-d', $this->global['timestamp'])) + 24 * 3600;
        if($owner['expirydate'] < $today) {
            //到期了删除记录
            $this->db->from('dbpre_mysubject');
            $this->db->where('id', $owner['id']);
            $this->db->delete();
            $this->update_owner($sid); //更新主题表
            //给用户发送提醒，提示已经到期
            $subject = $this->read($sid, 'name,subname', false);
            if(!$subject) return;
            $url = '<a href="'.url("item/detail/id/$sid").'" target="_blank">'.$subject['name'].'</a>';
            $note = lang('item_notice_owner_expirydate_out', $url);
            $this->loader->model('member:notice')->save($owner['uid'], 'subject', 'owner', $note);
        }
    }
    ...
    //读取单个详细样式主题
    function display_detailfield($subject,$nourl = false) {
        $modelid = $this->get_modelid($subject['pid']);
        //生成表格内容
        ...
        $fields = $this->variable('field_' . $modelid);
        foreach($fields as $val) {
            if(in_array($val['fieldname'], array('name','subname','mappoint','status','templateid','listorder'))) continue;
    ...
    function add_user_point($uid, $num = 1) {
        ...
        $this->loader->model('member:point')
            ->update_point($uid, 'add_subject', FALSE, $num, FALSE, array('subjects'=>array('set_add',array($num))));
    }
    ...
    function dec_user_point($uid, $num = 1) {
        if(!$uid) return;
        $this->loader->model('member:point')
            ->update_point($uid, 'add_subject', TRUE, $num, FALSE, array('subjects'=>array('set_add',array($num))));
    }
    ...
    function _delete_field($ids, $pid) {
        $this->get_category($pid);
        $this->get_model($this->category['modelid']);
        if(!$this->model['tablename']) return;
        $table = 'dbpre_' . $this->model['tablename'];
        //删除附表数据
        $this->db->from($table);
