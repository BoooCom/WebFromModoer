...
if(!defined('IN_ADMIN')) {
    $acts = array('mobile','manage','ajax','category','member','index','list','detail','pic','allpic','reviews',
	...
	if($_GET['top_appkey']&&$_GET['top_session']&&$_GET['top_parameters']) {
        if(strposex($_GET['top_session'],"\n")||strlen($_GET['top_session'])>60) redirect('无效字段！');
