...
switch ($op) {
    ...
    default:
        ...
        if($_GET['starttime']) $I->db->where_more('addtime', strtotime($_GET['starttime']));
        if($_GET['endtime']) $I->db->where_less('addtime', strtotime($_GET['endtime']));
        $I->db->where('status', 1);
        $total = $I->db->count();
        if($total) {
