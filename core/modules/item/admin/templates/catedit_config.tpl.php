    ...
    <div class="space">
        ...
        <table class="maintable" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr>
                ...
                <td><?=form_bool('t_cfg[enable_add]', isset($t_cfg['enable_add'])?$t_cfg['enable_add']:1)?></td>
            </tr>
            <tr>
                <td class="altbg1"><strong>允许主题关联主分类：</strong>允许会员在添加主题的时候仅关联主分类</td>
                <td><?=form_bool('t_cfg[relate_root]', isset($t_cfg['relate_root'])?$t_cfg['relate_root']:0)?></td>
            </tr>
            <tr>
            ...
            <tr>
                <td class="altbg1"><strong>允许游客留言：</strong>开启游客留言后，游客可以在不登录网站的前提下给<?=$t_mod['item_name']?>进行留言。</td>
                <td><?=form_bool('t_cfg[guest_gbook]', (bool)$t_cfg['guest_gbook'])?></td>
            </tr>
            <tr>
            ...
            <?if($_G['modules']['product'] && !$_G['modules']['product']['extra']):?>
            <?php  $pcfg = $_G['loader']->variable('config','product'); ?>
        ...
        <center>
            ...
            <button type="button" class="btn" onclick="jslocation('<?=cpurl($module,'category_list')?>')" /><?=lang('global_return')?></button>
        </center>
    </div>
</form>
</div>