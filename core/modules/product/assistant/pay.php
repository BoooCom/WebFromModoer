...
if(check_submit('dosubmit')) {
    ...
    if($_POST['payment'] == 'online') {
        $O->pay_online($orderid, _post('payment_name', null, MF_TEXT));
    } else {
        //密码验证
        if(!$_POST['password']) {
            redirect('对不起，您未填写帐号密码！');
        } elseif(!$user->check_passpord($_POST['password'])) {
            redirect('对不起，您填写的密码不正确！');
        }
        if($_POST['payment']=='cod') {
            $O->cod_submit($orderid);
        } else {
            $O->pay($orderid);
        }
        //支付完成后跳转
    ...
    if($op=='succeed' && $detail['status']>=2 and $detail['status']<=5) {
        ...
        $offlinepay = $O->get_offlinepay($detail['sid']);
