...
class msm_product_ordergoods extends ms_model {
    ...
    function savep($orderid, $products) {
        foreach ($products as $product) {
            $price = $this->loader->model(':product')->myprice($product);
            $this->db->from($this->table);
            $this->db->set('orderid', $orderid);
            $this->db->set('pid', $product['pid']);
            $this->db->set('pname', $product['subject']);
            $this->db->set('price', $price);
            $this->db->set('quantity', $product['quantity']);
            $this->db->set('goods_image', $product['thumb']);
            $this->db->insert();
            //增加产品销售数
            $this->sale_num_add($product['pid'], $product['quantity']);
        }
    }
