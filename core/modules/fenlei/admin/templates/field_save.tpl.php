...
<form method="post" action="<?=cpurl($module, $act, 'save')?>">
    <div class="space">
        <div class="subtitle">添加/编辑字段</div>
        <table class="maintable" border="0" cellspacing="1" cellpadding="1">
            ...
            <tr>
                ...
                <td><input type="text" class="txtbox4" name="t_field[unit]" value="<?=$t_field['unit']?>" /></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td><input type="text" class="txtbox2" name="t_field[regular]" id="regular" value="<?=$t_field['regular']?>" />&nbsp;
                    <select onchange="$('#regular').val($(this).val());">
                        <option value="" selected="selected">==常用表达式==</option>
                        <option value="/^[0-9]+$/">数字</option>
                        <option value="/^[a-zA-Z]+$/">字母</option>
                        <option value="/^[0-9a-zA-Z]+$/">数字+字母</option>
                        <option value="/^[\w\-\.]+@[\w\-\.]+(\.\w+)+$/">E-mail</option>
                        <option value="/^[0-9]{5,18}$/">QQ号</option>
                        <option value="/^(13|14|15|18)[0-9]{9}$/">手机号码</option>
        ...
        <center>
            ...
            <input type="hidden" name="t_field[id]" value="<?=$pid?>" />
            <input type="hidden" name="t_field[tablename]" value="<?=$tablename?>" />
            <input type="hidden" name="t_field[idtype]" value="fenlei" />
            <?else:?>
            <input type="hidden" name="pid" value="<?=$t_field['id']?>" />
            <input type="hidden" name="isedit" value="yes" />
            <input type="hidden" name="t_field[id]" value="<?=$t_field['id']?>" />
