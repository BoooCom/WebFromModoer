    ...
    <div class="space">
        ...
        <table class="maintable" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="starttime" value="<?=$detail['starttime']?date('Y-m-d',$detail['starttime']):''?>" class="txtbox3" />
                    -
                    <input type="text" name="endtime" value="<?=$detail['endtime']?date('Y-m-d',$detail['endtime']):''?>" class="txtbox3" />
                    (YYYY-MM-DD)
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <td class="altbg1">优惠简要说明:</td>
                <td><input type="text" name="des" value="<?=$detail['des']?>" class="txtbox" /></td>
            </tr>
            <?if($MOD['sendsms']):?>
            <tr>
                <td class="altbg1">手机短息发送内容:</td>
                <td>
                    <input type="text" name="sms_text" value="<?=$detail['sms_text']?>" class="txtbox" />
                    <div>
                        短信可使用标签(包括花括号):
                        <span title="优惠券编号">{id}</span>
                        <span title="发送短信会员昵称">{username}</span>
                        <span title="有效期开始时间">{starttime}</span>
                        <span title="有效期截止时间">{endtime}</span>
                    </div>
                </td>
            </tr>
            <?endif;?>
