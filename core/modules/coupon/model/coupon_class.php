...
class msm_coupon extends ms_model {
	...
	function init_field() {
		$this->add_field('city_id,catid,sid,starttime,endtime,subject,des,content,sms_text,status');
		$this->add_field_fun('catid,sid,starttime,endtime,status', 'intval');
        $this->add_field_fun('subject,des,sms_text', '_T');
    ...
    //发送短息息
    function sendsms($mobile, $coupon) {

        $P =& $this->loader->model('member:point');
        if(!$this->global['user']->isLogin) {
            if($points = $P->point['sendsms_coupon']) {
        ...
        if(!validate::is_mobile($mobile)) {
            redirect('对不起您未填写正确的手机号码。');
        }
        if(!is_array($coupon)) {
            $coupon = $this->read((int)$coupon);
        }
        if(!$coupon) redirect('coupon_empty');
        if(!$coupon['sms_text']) redirect('本条优惠券信息未提供没有手机短信优惠功能。');

        $username = $this->global['user']->isLogin ? $this->global['user']->username : '游客';
        $s = array('{id}','{username}','{starttime}','{endtime}');
        $r = array($coupon['couponid'], $username, date('Y-m-d', $coupon['starttime']), date('Y-m-d', $coupon['endtime']));
        $message = str_replace($s, $r, $coupon['sms_text']);
        //vp($message);

        $this->loader->model('sms:factory',false);
        $sms = msm_sms_factory::create();
        if(!$sms) redirect('手机短信发送接口不可用，请联系网站管理员。');
        if($sms->send($mobile, $message)) {
            $P->allow_del = false; //不允许扣除成为负数，即会员积分不足时，进行提示
            $P->update_point($this->global['user']->uid, 'sendsms_coupon', TRUE, 1);
            //增加发送次数
            $this->db->from($this->table);
            $this->db->set_add('send_num', 1);
            $this->db->where('couponid', $coupon['couponid']);
            $this->db->update();
            return TRUE;
        }
        return FALSE;
    ...
    function user_point_add($uid,$num) {
        ...
        $this->loader->model('member:point')
            ->update_point($uid, 'add_coupon', FALSE, $num);
    }

    function user_point_print($uid,$num) {
        if(!$uid) return;
        $this->loader->model('member:point')
            ->update_point($uid, 'print_coupon', TRUE, $num);
    }
    ...
    function print_coupon($couponid) {
        ...
        if(!$id) {
            $P =& $this->loader->model('member:point');
            $P->allow_del = false; //不允许扣除成为负数，即会员积分不足时，进行提示
            $P->update_point($this->global['user']->uid,'print_coupon',TRUE,1);
            unset($P);
        }
        //增加打印次数
        $this->db->from($this->table);
    ...
    function _feed_subject(&$FEED, &$detail) {
        ...
        $param = array();
        $param['flag'] = 'item';
        $param['uid'] = 0; //主题feed，不需要设定uid，username
        $param['username'] = '';
