...
class ms_upload_image extends ms_upload_file {
    ...
    var $watermark_limitsize = array(); //打水印图片尺寸限制
    ...
    function __construct($name, $exts = '') { //PHP 5
        ...
        //缩略图质量等级
        $this->set_thumb_level((int) _G('cfg','picture_createthumb_level'));
        //原图添加水印的尺寸限制
        $wlw = (int) _G('cfg','watermark_limitsize_width');
        $wlh = (int) _G('cfg','watermark_limitsize_height');
        $this->set_watermark_limitsize($wlw, $wlh);
    }
    ...
    //设置添加水印尺寸限制
    function set_watermark_limitsize($width,$height) {
        $this->watermark_limitsize['width'] = (int)$width;
        $this->watermark_limitsize['height'] = (int)$height;
    }
    ...
    function upload($folder, $subdir = 'MONTH', $mment = NULL, $delete_sorcue = FALSE) {
        ...
        */
        ...
        if($this->width >=  $this->watermark_limitsize['width'] && $this->height >= $this->watermark_limitsize['height']) {
            //watermark
            $w = MUDDER_ROOT . 'static' . DS . 'images' . DS . 'watermark.png';
            $this->waterimg = ($this->waterimg && is_file($this->waterimg)) ? $this->waterimg : $w;
            if(($this->userWatermark) && is_file($this->waterimg)) {
                $this->watermark($sorcuefile, $sorcuefile, $this->waterimg, $this->thumb_level);
            }
        }
