...
class ms_database extends ms_mysql {
    ...
    function get_table($tablename) {
        if(!preg_match("/^([a-z0-9_]+)$/i", $tablename)) redirect('无效的数据表名称:'._T($tablename));
    ...
    */
    ...
    //$table1参数格式为数组('表名'=>'关联字段ID')
    // function left_join($table1, $table2) {
    //     $table1 = $this->get_table($table1[0]);
    //     $key1 = $this->_ck_field($table1[1]);
    //     $table2 = $this->get_table($table2[0]);
    //     $key2 = $this->_ck_field($table2[1]);

    //     $this->from = "$table1 LEFT JOIN $table2 ON ({$table1}.{$key1}={$table2}.{$key2})";
    //     return $this;
    // }
    ...
    function where_exist($sql, $split='AND', $kh = '') {
        $where = 'exists(' .$this->repace_table($sql) . ')';
        $this->where .= ($this->where ? " $split " : '') . ($kh=='('?'(':'') . $where . ($kh==')'?')':'');
        return $this;
    }
    // 设置exist子查询
    function where_in_select($key, $sql, $split='AND', $kh = '') {
        $where = $key . ' IN(' .$this->repace_table($sql) . ')';
        $this->where .= ($this->where ? " $split " : '') . ($kh=='('?'(':'') . $where . ($kh==')'?')':'');
    }
    // 设置group_by子句
    ...
    function clear_table() {
        ...
        $this->exec("TRUNCATE TABLE " . $this->from);
    }

    //删除一个表
    function drop_table() {
        if(!$this->from) show_error(lang('global_sql_invalid', 'FROM(Dorp Table)'));
        $this->exec("DROP TABLE IF EXISTS " . $this->from);
    }
